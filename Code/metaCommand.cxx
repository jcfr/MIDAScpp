                            ...
                            METAIO_STL::vector<Field> fields)
{
  ...
  if(tag == "")
    {
    METAIO_STREAM::cout << "Tag cannot be empty : use AddField()\
                           instead." << METAIO_STREAM::endl;
    return false;
    }
                            ...
                            METAIO_STL::string defVal)
{
  ...
  if(tag == "")
    {
    METAIO_STREAM::cout << "Tag cannot be empty :\
                           use AddField()\
                           instead." << METAIO_STREAM::endl;
    return false;
    }
...
bool MetaCommand::GetValueAsBool(METAIO_STL::string optionName,
...
float MetaCommand::GetValueAsFloat(METAIO_STL::string optionName,
                                   METAIO_STL::string fieldName)
{
  METAIO_STL::string fieldname = fieldName;
  if(fieldName == "")
...
int MetaCommand::GetValueAsInt(METAIO_STL::string optionName,
                               METAIO_STL::string fieldName)
{
  METAIO_STL::string fieldname = fieldName;
  if(fieldName == "")
...
/** Return the value of the option as a string */
METAIO_STL::string MetaCommand::GetValueAsString(Option option,
                                                 METAIO_STL::string fieldName)
{
  METAIO_STL::string fieldname = fieldName;
  if(fieldName == "")
...
void MetaCommand::ListOptions()
{
  ...
  while(it != m_OptionVector.end())
    ...
    METAIO_STREAM::cout << "   Number of expeted values: " << 
                                 (*it).fields.size() << METAIO_STREAM::endl;
    ...
    while(itField != (*it).fields.end())
      ...
      METAIO_STREAM::cout << "      Description: " << 
                       (*itField).description.c_str() << METAIO_STREAM::endl;
      METAIO_STREAM::cout << "      Type: " << this->TypeToString(
                             (*itField).type).c_str() << METAIO_STREAM::endl;
      METAIO_STREAM::cout << "      Value: " << 
                             (*itField).value.c_str() << METAIO_STREAM::endl;
...
void MetaCommand::ListOptionsXML()
{
  ...
  while(it != m_OptionVector.end())
    ...
    METAIO_STREAM::cout << "<nvalues>" << (*it).fields.size() << 
                                         "</nvalues>" << METAIO_STREAM::endl;
    
    METAIO_STL::vector<Field>::const_iterator itField = (*it).fields.begin();
    while(itField != (*it).fields.end())
      {
      METAIO_STREAM::cout << "<field>" << METAIO_STREAM::endl;
      METAIO_STREAM::cout << "<name>" << (*itField).name.c_str() << 
                                             "</name>" << METAIO_STREAM::endl;
      METAIO_STREAM::cout << "<description>" << (*itField).description.c_str() 
                                   << "</description>" << METAIO_STREAM::endl;
      METAIO_STREAM::cout << "<type>" << this->TypeToString(
                 (*itField).type).c_str() << "</type>" << METAIO_STREAM::endl;
...
bool MetaCommand::ParseXML(const char* buffer)
{
  ...
  while(buf.size() > 0)
    ...
    for(unsigned int i=0;i<n;i++)
      ...
      field.type = this->StringToType(
                        this->GetXML(f.c_str(),"type",0).c_str());
...
bool MetaCommand::ExportGAD(bool dynamic)
{
  ...
  if(m_Name=="")
    {
    METAIO_STREAM::cout << "Set the name of the application\
                           using SetName()" << METAIO_STREAM::endl;
    return false;
    }
  ...
  if(!file.is_open())
    {
    METAIO_STREAM::cout << "Cannot open file for\
                   writing: " << filename.c_str() <<METAIO_STREAM::endl;
    return false;
    }
  ...
  file << " <componentAction type=\"JobSubmission\" order=\"" 
                                << order << "\">" << METAIO_STREAM::endl;
...
{  
  ...
  bool isComplete = false; // check if the option should be parse 
  ...
  for(unsigned int i=1;i<(unsigned int)argc;i++)
    ...
    if(argv[i][0] == '-' && (atof(argv[i]) == 0) && (strlen(argv[i])>1))
      ...
      if(valuesRemaining!=0)
        ...
        if(!isComplete)
          {
          METAIO_STREAM::cout << "Found tag before \
                                 end of value list!" << METAIO_STREAM::endl;
          return false;
          }
        else
          {
      ...
      if(!found && m_Verbose)
        {
        METAIO_STREAM::cout << "Too many arguments \
                               specified in your command line! "
                            << "Skipping extra argument: " 
                            << argv[i] << METAIO_STREAM::endl;
        }
    ...
    if(isComplete)
      ...
      if(currentOption >=0 && currentOption < (int)(m_OptionVector.size()))
        ...
        m_OptionVector[currentOption].fields[s-valuesRemaining].value
                                                           = argv[i];
  ...
  if(valuesRemaining>0)
    ...
    METAIO_STREAM::cout << "Options: " << METAIO_STREAM::endl
              << "  -v or -h for help listed in short format" 
              << METAIO_STREAM::endl
              << "  -V or -H for help listed in long format" 
              << METAIO_STREAM::endl
              << "  -vxml for help listed in xml format" 
              << METAIO_STREAM::endl
