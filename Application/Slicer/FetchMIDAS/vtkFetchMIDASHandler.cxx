...
vtkStandardNewMacro ( vtkFetchMIDASHandler );
vtkCxxRevisionMacro ( vtkFetchMIDASHandler, "$Revision$" );
...
size_t xnd_read_callback(void *ptr, size_t size, size_t nmemb, FILE *stream)
{
  ...
  std::cout << "*** vtkFetchMIDASHandler: We read " << retcode << " bytes from file\n";
...
//----------------------------------------------------------------------------
vtkFetchMIDASHandler::vtkFetchMIDASHandler()
{
}
...
//----------------------------------------------------------------------------
void vtkFetchMIDASHandler::PrintSelf(ostream& os, vtkIndent indent)
{
...
//--- for downloading
//----------------------------------------------------------------------------
void vtkFetchMIDASHandler::StageFileRead(const char * source,
...
//--- for uploading
//----------------------------------------------------------------------------
void vtkFetchMIDASHandler::StageFileWrite(const char *source,
                                   const char *destination )
...
//----------------------------------------------------------------------------
int vtkFetchMIDASHandler::PostTag ( const char *svr, const char *label,
...
//----------------------------------------------------------------------------
int vtkFetchMIDASHandler::PostMetadata( const char *serverPath,
...
//----------------------------------------------------------------------------
int vtkFetchMIDASHandler::DeleteResource ( const char *uri, const char *temporaryResponseFileName )
{
  int result;
  if (uri == NULL )
    {
  ...
  if (responseFile == NULL)
    {
    vtkErrorMacro("vtkFetchMIDASHandler::DeleteResource unable to open a local file caled " << responseFileName << " to write out to for capturing the uri");
    }
  else
    {
    // for windows
  ...
  if (retval == CURLE_OK)
    {
    result = 1;
    vtkDebugMacro("vtkFetchMIDASHandler::DeleteResource successful return from curl");
    }
  else
    {
    if (retval == CURLE_BAD_FUNCTION_ARGUMENT)
      {
      result = 0;
      vtkErrorMacro("vtkFetchMIDASHandler::DeleteResource bad function argument to curl, did you init CurlHandle?");
      }
    else if (retval == CURLE_OUT_OF_MEMORY)
      {
      result = 0;
      vtkErrorMacro("vtkFetchMIDASHandler::DeleteResource curl ran out of memory!");
      }
    else
      {
      result = 0;
      const char *returnString = curl_easy_strerror(retval);
      vtkErrorMacro("vtkFetchMIDASHandler::DeleteResource error running curl: " << returnString);
      }
    }
  this->CloseTransfer();
  fclose(this->LocalFile);
...
//--- for querying
//----------------------------------------------------------------------------
const char *vtkFetchMIDASHandler::QueryServer( const char *uri, const char *destination)
{
...
//----------------------------------------------------------------------------
const char* vtkFetchMIDASHandler::GetXMLDeclaration()
{
...
//----------------------------------------------------------------------------
const char* vtkFetchMIDASHandler::GetNameSpace()
{
