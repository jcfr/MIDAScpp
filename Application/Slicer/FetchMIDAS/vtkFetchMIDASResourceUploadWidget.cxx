...
//---------------------------------------------------------------------------
vtkStandardNewMacro (vtkFetchMIDASResourceUploadWidget );
vtkCxxRevisionMacro ( vtkFetchMIDASResourceUploadWidget, "$Revision$");


//---------------------------------------------------------------------------
vtkFetchMIDASResourceUploadWidget::vtkFetchMIDASResourceUploadWidget ( )
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::PrintSelf ( ostream& os, vtkIndent indent )
{
    this->vtkObject::PrintSelf ( os, indent );

    os << indent << "vtkFetchMIDASResourceUploadWidget: " << this->GetClassName ( ) << "\n";
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::SetStatusText (const char *txt)
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::DestroyNewTagWindow( )
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::WithdrawNewTagWindow ( )
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::RaiseNewTagWindow(const char *att)
{
  ...
  if ( this->Logic->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ("RaiseNewTagWindow: Got NULL FetchMIDASNode");
    return;
    }
  ...
  if ( this->NewTagWindow == NULL )
    ...
    this->AddNewValueButton->SetImageToIcon ( this->FetchMIDASIcons->GetAddNewIcon() );
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::UpdateNewUserTag( const char *att, const char *val)
{
  ...
  if ( this->Logic->GetFetchMIDASNode() == NULL) 
    {
    vtkErrorMacro ( "vtkFetchMIDASResourceUploadWidget: FetchMIDASNode is NULL.");
    this->ResetCurrentTagLabel();
    return;
    }
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::ProcessWidgetEvents ( vtkObject *caller, unsigned long event, void *callData )
{
  ...
  if ( this->IsCreated() )
    ...
    if ( (b == this->GetApplyTagsButton()) && (event == vtkKWPushButton::InvokedEvent ) )
      {
      this->InvokeEvent (vtkFetchMIDASResourceUploadWidget::TagSelectedDataEvent);
      this->InvokeEvent (vtkFetchMIDASResourceUploadWidget::ShowAllTagViewEvent );      
      }
    else if ( (b == this->GetRemoveTagsButton()) && (event == vtkKWPushButton::InvokedEvent ) )
      {
      this->InvokeEvent (vtkFetchMIDASResourceUploadWidget::RemoveTagSelectedDataEvent);
      }
      ...
      {
      this->InvokeEvent ( vtkFetchMIDASResourceUploadWidget::UploadRequestedEvent );
      }
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::RaiseTaggingHelpWindow()
{
  int px, py;
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::SelectRow( int i)
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::DeselectRow( int i)
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::SelectAllItems()
{
  int numrows, i;
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::DeselectAllItems()
{
  int numrows, i;
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::UpdateSelectedStorableNodes()
{
  int numrows, i;
...
//---------------------------------------------------------------------------
int vtkFetchMIDASResourceUploadWidget::IsItemSelected(int i)
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::DeleteSelectedItems()
{
 int r = this->GetMultiColumnList()->GetWidget()->GetNumberOfRows();
  for ( int i=0; i < r; i++)
    {
...
//---------------------------------------------------------------------------
int vtkFetchMIDASResourceUploadWidget::GetNumberOfSelectedItems()
{
  int r = this->GetMultiColumnList()->GetWidget()->GetNumberOfRows();
  int numSelected = 0;
  for ( int i=0; i < r; i++)
...
//---------------------------------------------------------------------------
int vtkFetchMIDASResourceUploadWidget::GetNumberOfItems()
{
...
//---------------------------------------------------------------------------
const char* vtkFetchMIDASResourceUploadWidget::GetNthDataTarget(int i )
{
  int r = this->GetMultiColumnList()->GetWidget()->GetNumberOfRows();
  if ( i >= 0 && i < r )
    {
...
//---------------------------------------------------------------------------
const char* vtkFetchMIDASResourceUploadWidget::GetNthSlicerDataType(int i)
{
  int r = this->GetMultiColumnList()->GetWidget()->GetNumberOfRows();
...
//---------------------------------------------------------------------------
const char* vtkFetchMIDASResourceUploadWidget::GetNthSelectedSlicerDataType(int n)
{
  int r = this->GetMultiColumnList()->GetWidget()->GetNumberOfRows();
  int counter = 0;
...
//---------------------------------------------------------------------------
const char* vtkFetchMIDASResourceUploadWidget::GetNthSelectedDataTarget(int n)
{
 int r = this->GetMultiColumnList()->GetWidget()->GetNumberOfRows();
  int counter = 0;
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::ProcessMRMLEvents ( vtkObject *caller,
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::AddMRMLObservers ( )
{
}

//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::RemoveMRMLObservers ( )
{
}

//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::UpdateWidget()
{
}


//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::UpdateMRML()
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::RemoveWidgetObservers ( )
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::AddWidgetObservers ( )
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::CreateWidget ( )
{
  ...
  this->ShowTagsForAllButton->SetImageToIcon ( this->FetchMIDASIcons->GetShowDataTagsIcon() );
  ...
  this->ApplyTagsButton->SetImageToIcon ( this->FetchMIDASIcons->GetApplyTagsIcon() );
  ...
  this->RemoveTagsButton->SetImageToIcon ( this->FetchMIDASIcons->GetRemoveTagsIcon() );
  ...
  this->UploadButton->SetBorderWidth ( 0 );
  this->UploadButton->SetReliefToFlat();
  this->UploadButton->SetImageToIcon ( this->FetchMIDASIcons->GetUploadIcon() );
  ...
  this->TaggingHelpButton->SetImageToIcon ( this->FetchMIDASIcons->GetHelpIcon() );
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::PopulateTagMenuButtonCallback ()
{
  //--- populate this menu button from logic's structure.
  if ( this->Logic == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASResourceUploadWidget: PopulateTagMenuButtonCallback got NULL Logic.");
    return;
    }
  if ( this->Logic->GetFetchMIDASNode() == NULL)
    {
    vtkErrorMacro ("vtkFetchMIDASResourceUploadWidget: PopulateTagMenuButtonCallback got NULL FetchMIDASNode.");
    return;
    }
...
//----------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::RightClickListCallback(int row, int col, int x, int y)
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::AddNewItem ( const char *dataset, const char *dtype )
{
  ...
  if  (this->MRMLScene == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASResourceUploadWidget::AddNewItem: Got NULL MRMLScene." );
    return;
    }
  ...
  if ( unique )
    ...
    this->GetMultiColumnList()->GetWidget()->SetCellImageToIcon(i, 1, this->FetchMIDASIcons->GetShowDataTagsIcon() );
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::ShowTagsForSelection (  )
{
  vtkWarningMacro ("ShowTagsForSelection.");
  this->InvokeEvent ( vtkFetchMIDASResourceUploadWidget::ShowSelectionTagViewEvent );
}



//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::ShowTagViewCallback( )
{
  vtkKWMessageDialog *dialog = vtkKWMessageDialog::New();
  dialog->SetParent (this->GetParent() );
  dialog->SetStyleToMessage();
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::ResetCurrentTagLabel ( )
{
...
//---------------------------------------------------------------------------
const char *vtkFetchMIDASResourceUploadWidget::GetCurrentTagAttribute()
{
  return ( this->CurrentTagAttribute.c_str() );
}

//---------------------------------------------------------------------------
const char *vtkFetchMIDASResourceUploadWidget::GetCurrentTagValue()
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::SetCurrentTagAttribute(const char* att)
{
  if ( att != NULL )
    {
...
//---------------------------------------------------------------------------
void vtkFetchMIDASResourceUploadWidget::SetCurrentTagValue(const char* val)
{
  std::stringstream ss;
