...
#include "vtkFetchMIDAS.h"

#include "vtkFetchMIDASWriterXND.h"
...
#include "MIDAScpp/RestBitstream.h"
...
vtkFetchMIDASLogic* vtkFetchMIDASLogic::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkFetchMIDASLogic");
  if(ret)
    {
      return (vtkFetchMIDASLogic*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkFetchMIDASLogic;
}
...
//----------------------------------------------------------------------------
vtkFetchMIDASLogic::vtkFetchMIDASLogic()
{
  this->rest = RestAPI::New();
  //this->r->SetServerUrl("http://localhost/Midas/midas/");
  this->rest->SetOutputMode(RestAPIUtils::BUFFER);
  this->rest->SetInputMode(RestAPIUtils::BUFFER);
  /*this-> r->BuildTree("mrml");
  //std::cout << "***** Recherche brute *****\n";
  //this-> r->PrintTree(std::cout);
  //std::cout << "***** Filtrage mrml *****\n";
  this-> r->FilterExtension("mrML");
  this-> r->PrintTree(std::cout);

  std::vector<RestBitstream> l = r->GetBitstreams();
  for(int i=0; i<l.size(); i++)
  l[i].Print(std::cout, 0);*/
  ...
  this->ServerCollection = vtkFetchMIDASServerCollection::New();
...
//  this->DebugOn();
   ...
   vtkFetchMIDASWriterXND *xndw = vtkFetchMIDASWriterXND::New();
   vtkFetchMIDASParserXND *xndp = vtkFetchMIDASParserXND::New();
   vtkFetchMIDASWebServicesClientMIDAS *xndc = vtkFetchMIDASWebServicesClientMIDAS::New();

   //--- Add all known servers and set their service type.
   //--- Extend here as we support new web services.
   const char *IDString = "MIDAS";
   const char *HandlerString = "FetchMIDASHandler";
   vtkFetchMIDASServer *s1 = vtkFetchMIDASServer::New();
   s1->SetParser ( xndp );
   s1->SetWriter ( xndw );
   s1->SetWebServicesClient ( xndc );
   s1->SetName ( "http://localhost/Midas/midas" );
   ...
   vtkFetchMIDASServer *s2 = vtkFetchMIDASServer::New();
   s2->SetParser ( xndp);
   s2->SetWriter ( xndw);
   s2->SetWebServicesClient (xndc);
   s2->SetName ( "http://ij.itk.org/midas/");
   ...
   vtkFetchMIDASServer *s3 = vtkFetchMIDASServer::New();
   ...
   */
   ...
   if ( this->FetchMIDASNode != NULL )
     {
     this->FetchMIDASNode->InvokeEvent ( vtkMRMLFetchMIDASNode::KnownServersModifiedEvent );
     }
...
{ 
  ...
  if ( this->ServerCollection != NULL )
    ...
    for ( int i=0; i < num; i++ )
      {
      vtkFetchMIDASServer *s = vtkFetchMIDASServer::SafeDownCast (this->ServerCollection->GetItemAsObject (i) );
      if ( s )
        {
        if ( s->GetParser() )
          {
...
//----------------------------------------------------------------------------
vtkIntArray* vtkFetchMIDASLogic::NewObservableEvents()
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::InitializeInformatics ( )
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetHandlersOnServers ( )
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::CreateTemporaryFiles ( )
{
      ...
      if ( this->GetMRMLScene()->GetCacheManager()->GetRemoteCacheDirectory() != NULL )
        ...
        if ( this->XMLDirName.c_str() != NULL )
          ...
          if (vtksys::SystemTools::FileIsDirectory( this->XMLDirName.c_str() ))
            ...
            pathComponents.push_back("FetchMIDAS_QueryResponse.xml");
            ...
            pathComponents.push_back("FetchMIDAS_UploadMetadata.xml");
            ...
            //--- cache dir.
            pathComponents.pop_back();
            pathComponents.push_back("FetchMIDAS_TemporaryResponse.xml");
            ...
            //--- cache dir.
            pathComponents.pop_back();
            pathComponents.push_back("FetchMIDAS_DocumentDeclaration.xml");
            ...
            //--- cache dir.
            pathComponents.pop_back();
            pathComponents.push_back("FetchMIDAS_UploadHeader.xml");
            ...
            pathComponents.pop_back();
            pathComponents.push_back("FetchMIDAS_MRMLCache.xml");
        ...
        this->HTTPResponseFileName = std::string("FetchMIDAS_QueryResponse.xml");
        this->XMLUploadFileName = std::string("FetchMIDAS_UploadMetadata.xml");
        this->MRMLCacheFileName = std::string("FetchMIDAS_MRMLCache.xml");
        this->TemporaryResponseFileName = std::string ("FetchMIDAS_TemporaryResponse.xml");
        }
      }
    }
}
...
//----------------------------------------------------------------------------
const char *vtkFetchMIDASLogic::GetServiceTypeForServer ( const char *svc )
{
  if ( this->ServerCollection )
    {
    vtkFetchMIDASServer *s = this->ServerCollection->FindServerByName ( svc );
    if ( s )
      {
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::AddNewServer (const char *name,
                                    const char *type,
...

{
  if ( name == NULL )
    {
    vtkErrorMacro ( "vtkFetchMIDASLogic::AddNewServer: got NULL Server name.");
    return;
    }
  if ( URIHandlerName == NULL )
    {
    vtkErrorMacro ( "vtkFetchMIDASLogic::AddNewServer: got NULL URIHandler name.");
    return;
    }
  if ( TagTableName == NULL )
    {
    vtkErrorMacro ( "vtkFetchMIDASLogic::AddNewServer: got NULL TagTable name");
    return;
    }
  // Developers note: extend this check as new service types are added.
  if ( !(strcmp(type, "MIDAS")))
    {
    vtkFetchMIDASWebServicesClientMIDAS *wsc = vtkFetchMIDASWebServicesClientMIDAS::New();
    wsc->SetName(name);
    ...
    s1->SetWebServicesClient ( wsc );
    s1->SetURIHandlerName ( "FetchMIDASHandler" );
    this->ServerCollection->AddItem ( s1 );
    this->SetCurrentWebService(s1);
    s1->Delete();
...
//---------------------------------------------------------------------------
void vtkFetchMIDASLogic::RequestResourceUpload ( )
{
  //---
  ...
  if ( this->GetMRMLScene() == NULL )
    ...
    this->FetchMIDASNode->SetErrorMessage (msg.c_str() );
    this->FetchMIDASNode->InvokeEvent ( vtkMRMLFetchMIDASNode::RemoteIOErrorEvent );
    return;
    }
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ( "RequestResourceUpload: FetchMIDASNode is NULL.");
    std::string msg = "No upload initiated; upload methods are not properly configured.";
    this->FetchMIDASNode->SetErrorMessage (msg.c_str() );
    this->FetchMIDASNode->InvokeEvent ( vtkMRMLFetchMIDASNode::RemoteIOErrorEvent );
    return;
...
//---------------------------------------------------------------------------
int vtkFetchMIDASLogic::RequestStorableNodesUpload ( )
{
...
//---------------------------------------------------------------------------
int vtkFetchMIDASLogic::RequestSceneUpload ( )
{ 
...
//---------------------------------------------------------------------------
int vtkFetchMIDASLogic::AddNewTagsToServer ( )
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASLogic::SaveResourceSelectionState ( )
{
  //--- Temporary Save All
...
//---------------------------------------------------------------------------
void vtkFetchMIDASLogic::RestoreResourceSelectionState ( )
{
...
//---------------------------------------------------------------------------
int vtkFetchMIDASLogic::WriteDocumentDeclaration ( )
{
...
//---------------------------------------------------------------------------
int vtkFetchMIDASLogic::WriteMetadataForUpload (const char *nodeID )
{
  //  return 1 if ok, 0 if not.
  if ( this->FetchMIDASNode == NULL) 
    {
    vtkErrorMacro ( "WriteMetadataForUpload: FetchMIDASNode is NULL.");
    return 0;
    }
  if (this->MRMLScene == NULL )
    {
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetFetchMIDASNode( vtkMRMLFetchMIDASNode *node )
{
  if ( node != this->FetchMIDASNode )
    {
    vtkIntArray *events = vtkIntArray::New();
    events->InsertNextValue ( vtkMRMLFetchMIDASNode::SelectedServerModifiedEvent );
    events->InsertNextValue ( vtkMRMLScene::NodeAddedEvent );
    vtkSetAndObserveMRMLNodeEventsMacro ( this->FetchMIDASNode, node, events );
    events->Delete();
    }
}
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ProcessMRMLEvents ( vtkObject *caller, unsigned long event, void *callData )
{
  ...
  if ( this->FetchMIDASNode == NULL )
    {
    vtkErrorMacro ( "FetchMIDASLogic::ProcessMRMLEvents: got null FetchMIDASNode." );
    return;
    }
  if ( this->FetchMIDASNode->GetTagTableCollection() == NULL )
    {
    vtkErrorMacro ( "FetchMIDASLogic::ProcessMRMLEvents: got null FetchMIDASNode TagTableCollection." );
    return;
    }
  ...
  if ( node == this->FetchMIDASNode && event == vtkMRMLFetchMIDASNode::SelectedServerModifiedEvent )
    {
    this->CurrentWebService = this->GetServerCollection()->FindServerByName ( this->FetchMIDASNode->GetSelectedServer() );
    this->CurrentWebService->SetTagTable (this->FetchMIDASNode->GetTagTableCollection()->
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::DeselectNode( const char *nodeID)
{
  std::string s;
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SelectNode( const char *nodeID)
{
  int unique = 1;
  std::string s;
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SelectScene()
{
  if ( this->FetchMIDASNode == NULL )
    {
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::DeselectScene()
{
  if ( this->FetchMIDASNode == NULL )
    {
    vtkErrorMacro ( "vtkFetchMIDASLogic: FetchMIDASNode is NULL.");
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ApplySlicerDataTypeTag()
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetSlicerDataTypeOnVolumeNodes()
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetSlicerDataTypeOnModelNodes()
{
  if ( this->FetchMIDASNode == NULL )
    {
    return;
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetSlicerDataTypeOnFiberBundleNodes()
{
  if ( this->FetchMIDASNode == NULL )
    {
    return;
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetSlicerDataTypeOnUnstructuredGridNodes()
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetSlicerDataTypeOnFiducialListNodes()
{
  if ( this->FetchMIDASNode == NULL )
    {
    return;
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetSlicerDataTypeOnColorTableNodes()
{
  if ( this->FetchMIDASNode == NULL )
    {
    return;
  ...
  for (n=0; n<nnodes; n++)
    ...
    this->FetchMIDASNode->InvokeEvent ( vtkMRMLFetchMIDASNode::NewNodeTaggedEvent );
    }
}
...
//----------------------------------------------------------------------------
const char *vtkFetchMIDASLogic::GetHTTPResponseFileName ( )
{
...
//----------------------------------------------------------------------------
const char* vtkFetchMIDASLogic::GetXMLHeaderFileName ( )
{
...
//----------------------------------------------------------------------------
const char* vtkFetchMIDASLogic::GetXMLDocumentDeclarationFileName ( )
{
...
//----------------------------------------------------------------------------
const char *vtkFetchMIDASLogic::GetXMLUploadFileName ( )
{
...
//----------------------------------------------------------------------------
const char *vtkFetchMIDASLogic::GetTemporaryResponseFileName ( )
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::QueryServerForTags ( )
{
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ( "QueryServerForTags: FetchMIDASNode is NULL.");
  ...
  if ( svrName == NULL || !(strcmp(svrName, "") ) )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic: Null or empty server name." );
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::QueryServerForTagValues ( )
{
  ...
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ( "QueryServerForTagValues: FetchMIDASNode is NULL.");
  ...
  this->FetchMIDASNode->InvokeEvent ( vtkMRMLFetchMIDASNode::TagResponseReadyEvent );          
}
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::UpdateMRMLQueryTags()
{
...
//----------------------------------------------------------------------------
unsigned int vtkFetchMIDASLogic::GetNumberOfURIsDeletedOnServer ( )
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ResetURIsDeletedOnServer ( )
{
...
//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::DeleteSceneFromServer ( const char *uri )
{
  int retval = 1;
  ...
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ( "DeleteSceneFromServer: FetchMIDASNode is NULL.");
    return 0;
    }
  if ( this->GetMRMLScene()->GetCacheManager() == NULL )
    {
  ...
  if ( this->GetMRMLScene()->GetCacheManager()->GetFilenameFromURI(uri) != NULL )
    ...
    std::string tmp = "FetchMIDASDeleteTargetScene.mrml";
    std::vector<std::string> pathComponents;
...
//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::DeleteResourceFromServer ( const char *uri )
{
  int retval = 1;
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::QueryServerForResources ( )
{
  //--- get selected tags
  ...
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ( "QueryServerForResources: FetchMIDASNode is NULL.");
    return;
    }
  if ( this->CurrentWebService == NULL )
    {
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ParseValuesForTagQueryResponse ( const char *att )
{
  if ( this->GetCurrentWebService()->GetParser () )
    {
    this->GetCurrentWebService()->GetParser()->SetFetchMIDASNode ( this->FetchMIDASNode );
    ...
    this->GetCurrentWebService()->GetParser()->SetFetchMIDASNode ( NULL );
    }
}
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ParseTagQueryResponse ( )
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ParseResourceQueryResponse ( )
{
  if ( this->GetCurrentWebService()->GetParser() )
    {
    this->GetCurrentWebService()->GetParser()->SetFetchMIDASNode ( this->FetchMIDASNode );
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::RefreshTags()
{
  this->ClearExistingTagsForQuery();
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::RefreshValuesForTag( const char *tagname )
{
  if ( this->CurrentWebService == NULL )
    {
  ...
  if ( this->FetchMIDASNode == NULL )
    {
    vtkErrorMacro ( "RefreshValuesForTag: Got NULL FetchMIDASNode." );
    return;
    }
  if ( this->CurrentWebService->GetTagTable() == NULL )
    {
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ClearExistingTagsForQuery ( )
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ClearExistingValuesForTag ( const char *tagname )
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::AddUniqueTag ( const char *tagname )
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::AddUniqueValueForTag ( const char *tagname, const char *val )
{
...
//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::GetNumberOfTagValues( const char *tagname)
{
...
//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::GetNumberOfTagsOnServer ( )
{
...
//----------------------------------------------------------------------------
const char * vtkFetchMIDASLogic::GetNthTagAttribute ( int n)
{
...
//----------------------------------------------------------------------------
const char * vtkFetchMIDASLogic::GetNthValueForTag(const char *tagname, int n)
{
...
//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::IsTagOnServer ( const char *tagname )
{
  std::map<std::string, std::vector<std::string> >::iterator iter;
  for ( iter = this->CurrentWebServiceMetadata.begin();
        iter != this->CurrentWebServiceMetadata.end();
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::GetCurrentListOfStorableNodes ( )
{
}


//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::CheckStorableNodesForTags ( )
{
  return 1;
}
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::TagStorableNodes ( )
{
}


//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::RequestResourceDownload ( const char *uri, const char *slicerDataType)
{
  if ( this->GetCurrentWebService() == NULL )
    {
  ...
  if ( uri == NULL || slicerDataType == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic: Null uri or slicerDataType." );
    return;
    }
  ...
  std::cout << "In " << __FILE__ <<", line " << __LINE__ << ": " << "pass" << std::endl;
  if ( !(strcmp(slicerDataType, "MRML")))
    {
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::RequestStorableNodeDownload ( const char *uri, const char *slicerDataType )
{
  ...
  if (uri == NULL || slicerDataType == NULL)
    {
    vtkErrorMacro("vtkFetchMIDASLogic::RequestResourceDownload: uri isn't set or slicerDataType isn't set. uri = " << (uri == NULL ? "null" : uri) << ", slicerDataType = " << (slicerDataType == NULL ? "null" : slicerDataType));
    return;
    }
  ...
  if ( this->GetMRMLScene() == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic::RequestResourceDownload: Null scene. ");
    return;
    }
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic::RequestResourceDownload: Null FetchMIDAS node. ");
    return;
    }
  ...
  if ( handler == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic::RequestResourceDownload: Null URIHandler. ");
    return;
    }
  ...
  if ((index = uriString.find("://", 0) ) != std::string::npos)
    ...
    vtkDebugMacro("vtkFetchMIDASLogic::RequestResourceDownload: new uri string = " << uriString.c_str());
    }
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::RequestSceneDownload ( const char *uri )
{
  ...
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ( "RequestSceneDownload: FetchMIDASNode is NULL.");
    std::string msg = "No download initiated; download methods are not properly configured.";
    this->FetchMIDASNode->SetErrorMessage (msg.c_str() );
    this->FetchMIDASNode->InvokeEvent ( vtkMRMLFetchMIDASNode::RemoteIOErrorEvent );
    return;
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ClearModifiedNodes()
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::AddModifiedNode( const char *nodeID)
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::RemoveModifiedNode( const char *nodeID)
{
  unsigned int z = this->ModifiedNodes.size();
  for (unsigned int i=0; i < z; i++ )
    {
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::ClearSelectedStorableNodes ()
{
  this->SelectedStorableNodeIDs.clear();
}
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::AddSelectedStorableNode( const char *nodeID)
{
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::RemoveSelectedStorableNode( const char *nodeID)
{
  unsigned int z = this->SelectedStorableNodeIDs.size();
  for (unsigned int i=0; i < z; i++ )
    {
...
//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::CheckStorageNodeFileNames()
{
...
//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::CheckModifiedSinceRead()
{
  // Before upload, check to make sure all storable nodes have
  // set storage nodes with filenames
...
//---------------------------------------------------------------------------
void vtkFetchMIDASLogic::SaveNewURIOnSelectedResource ( const char *olduri, const char *newuri )
{
...
//---------------------------------------------------------------------------
void vtkFetchMIDASLogic::SaveOldURIsOnSelectedResources()
{
  if ( this->GetMRMLScene() == NULL )
    {
  ...
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ("RestoreFileNamesOnSelectedResources Null FetchMIDAS node. ");
    return;
    }
...
//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::RestoreURIsOnSelectedResources ( )
{
  if ( this->GetMRMLScene() == NULL )
    {
  ...
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ("RestoreURIsOnSelectedResources Null FetchMIDAS node. ");
    return 0;
    }
...
//----------------------------------------------------------------------------
int vtkFetchMIDASLogic::RestoreFileNamesOnSelectedResources ( )
{
  if ( this->GetMRMLScene() == NULL )
    {
    vtkErrorMacro ("RestoreFileNamesOnSelectedResources Null scene. ");
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetCacheFileNamesOnSelectedResources ( )
{
  //---
  ...
  if ( this->GetMRMLScene() == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic::SetCacheFileNamesOnSelectedResources Null scene. ");
    return;
    }
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic::SetCacheFileNamesOnSelectedResources Null FetchMIDAS node. ");
    return;
    }
  ...
  if ( this->SceneSelected )
    ...
    vtkDebugMacro("vtkFetchMIDASLogic::SetCacheFileNamesOnSelectedResources setting scene url to " << mrmlFileName);
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetIdleWriteStateOnSelectedResources ( )
{
    if ( this->GetMRMLScene() == NULL )
    {
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetTransferDoneWriteStateOnSelectedResources ( )
{
    if ( this->GetMRMLScene() == NULL )
    {
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetCancelledWriteStateOnSelectedResources ( )
{
    if ( this->GetMRMLScene() == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic::SetCancelledWriteStateOnSelectedResources: Null scene. ");
    return;
    }
...
//----------------------------------------------------------------------------
void vtkFetchMIDASLogic::SetURIHandlerOnSelectedResources(vtkURIHandler *handler)
{
  //---
  // Get all selected storable nodes from this->SelectedStorableNodeIDs;
  // (This vector of strings is populated by the GUI when upload button is
  ...
  if ( handler == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic::SetURIHandlerOnSelectedResources: Null URIHandler. ");
    return;
    }
    if ( this->GetMRMLScene() == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic::SetURIHandlerOnSelectedResources: Null scene. ");
    return;
    }
  if ( this->GetFetchMIDASNode() == NULL )
    {
    vtkErrorMacro ("vtkFetchMIDASLogic::SetURIHandlerOnSelectedResources: Null FetchMIDAS node. ");
    return;
    }
...
//----------------------------------------------------------------------------
int  vtkFetchMIDASLogic::PostStorableNodes()
{
  // Get all selected storable nodes from this->SelectedStorableNodeIDs;
